...
class CI_Session_files_driver extends CI_Session_driver implements SessionHandlerInterface {
    ...
    public function gc($maxlifetime): bool
    {
        ...
        while (($file = readdir($directory)) !== FALSE) {
            if (!preg_match($pattern, $file) OR !is_file($this->_config['save_path'].'/'.$file)) {
                continue;
            }

            if (($file_time = filemtime($this->_config['save_path'].'/'.$file)) === FALSE) {
                continue;
            }

            if ($file_time < $ts) {
                unlink($this->_config['save_path'].'/'.$file);
            }
        }
    ...
    /**
     * Get length of string for mbstring or normal string
     *
