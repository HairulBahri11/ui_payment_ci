...
$pdf->Image('assets/logoui.png', 28, 0, 20, 20);

// Add watermark text
$pdf->SetFont('Arial', 'B', 50);
$pdf->SetTextColor(245, 245, 245); // Light gray color for watermark
$pdf->Cell($width, 150,  'Copy', 0, 0, 'C', false);
...
$query1 = "select py.total, py.method, py.number, py.bank, py.trfdate, pd.id, pd.paymentid, pd.studentid, pd.voucherid, pd.category, pd.monthpay, SUM(pd.amount) as subtotal, py.total, s.name, p.program, pd.explanation
                           FROM paydetail pd
...
$grandtotal = $row1->row()->total;

foreach ($row1->result() as $data1) {
	//$grandtotal += $data1->total;

	$pdf->ln($ln);
	$pdf->Cell($width / 2, 5, 'ID', 0, 0, 'L', false);
	$pdf->Cell($width / 2, 5, $data1->studentid, 0, 0, 'R', false);
	$pdf->ln($ln);
	$pdf->Cell($width / 2, 5, 'NAME', 0, 0, 'L', false);
	$nama_lengkap = $data1->name;
	$namaaja = substr($nama_lengkap, 0, 15);
	$pdf->Cell($width / 2, 5, $namaaja, 0, 0, 'R', false);
	$pdf->ln($ln);
	...
	if ($data1->method == 'DEBIT' || $data1->method == 'CREDIT' || $data1->method == 'SWITCHING CARD' || $data1->method == 'QRIS') {
		$pdf->ln($ln);
	...
	foreach ($row2 as $data2) {
		...
		if ($data2->category == 'COURSE') {
			$var = $data2->monthpay;
			$parts = explode('-', $var);
			$monthpay =  date("M", strtotime($data2->monthpay));
			$yearpay =  date("y", strtotime($data2->monthpay));
			$pdf->Cell($width / 2, 5, $data2->category . " ( " . $monthpay . " " . $yearpay . " )", 0, 0, 'R', false);
		} else {
			$pdf->Cell($width / 2, 5, $data2->category, 0, 0, 'R', false);
		}
		$pdf->ln($ln);
		$pdf->Cell($width / 2, 5, '', 0, 0, 'L', false);
		$pdf->Cell($width / 2, 5, "Rp. " . number_format($data2->amount, 0, ',', '.') . ",00", 0, 0, 'R', false);
		$total += $data2->amount;
		$i++;
	}
}
...
/*if($total-$grandtotal != 0){
    $pdf->ln($ln);
    $pdf->Cell($width/2,5,'Expense',0,0,'L',false);
    $pdf->Cell($width/2,5,"Rp. ".number_format($total-$grandtotal,0,',','.').",00",0,0,'R',false);
}*/

$pdf->ln(10);
$pdf->Cell($width / 2, 5, 'Total', 0, 0, 'L', false);

$pdf->Cell($width / 2, 5, "Rp. " . number_format($grandtotal, 0, ',', '.') . ",00", 0, 0, 'R', false);
//$pdf->Cell($width/2,5,"Rp. ".number_format($total,0,',','.').",00",0,0,'R',false);
$pdf->ln($ln + 2);
