...
$pdf->Cell($width, 110,  'Copy', 0, 0, 'C', false);
...
$query1 = "select py.total, py.method, py.number, py.bank, py.trfdate, pd.id, pd.paymentid, pd.studentid, pd.voucherid, pd.category, pd.monthpay, SUM(pd.amount) as subtotal, s.name, p.program, pd.explanation
                           FROM paydetail pd
...
foreach ($row1 as $data1) {
	...
	$nama_print = implode(" ", array_slice(explode(" ", $namaaja), 0, 3));
	$pdf->Cell($width / 2, 5, $nama_print, 0, 0, 'R', false);
	$pdf->ln($ln);
	...
	if ($data1->method == "BANK TRANSFER") {
		$pdf->ln($ln);
		$pdf->Cell($width / 2, 5, 'TRANSFER DATE', 0, 0, 'L', false);
		$pdf->Cell($width / 2, 5, $data1->trfdate, 0, 0, 'R', false);
	} elseif ($data1->method == 'DEBIT' || $data1->method == 'CREDIT' || $data1->method == 'SWITCHING CARD' || $data1->method == 'QRIS') {
		$pdf->ln($ln);
	...
	foreach ($row2 as $data2) {
		...
		if ($data2->category == 'COURSE') {
			...
			$pdf->Cell($width / 1, 5, $data2->category, 0, 0, 'R', false);
			$pdf->ln($ln);
			$pdf->Cell($width / 1, 5, $data2->explanation, 0, 0, 'R', false);
		} else {
...
$pdf->Cell($width / 2, 5, "Rp. " . number_format($data1->total, 0, ',', '.') . ",00", 0, 0, 'R', false);
$pdf->ln($ln + 2);
